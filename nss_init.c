...
static int __init nss_init(void)
{
	...
	spin_lock_init(&(nss_top_main.cm_lock));
	spin_lock_init(&(nss_top_main.crypto_lock));
	spin_lock_init(&(nss_top_main.wlan_lock));
	for (i = 0; i < NSS_MAX_CORES; i++) {
		spin_lock_init(&(nss_top_main.profiler_lock[i]));
	}
	spin_lock_init(&(nss_top_main.ipsec_lock));
	for (i = 0; i < NSS_MAX_PHYSICAL_INTERFACES; i++) {
		spin_lock_init(&(nss_top_main.phys_if_lock[i]));
	}
